...
router.get('/', function(req, res, next) {      
    dbConn.query('SELECT kode.Nama_Kec,kode.Nama_Kel, company.* FROM company INNER JOIN kode ON (company.kecamatan=kode.Kec AND company.kelurahan=kode.Kel)',function(err,rows)     {
        if(err) {
            req.flash('error', err);
            // render to views/users/index.ejs
            res.render('perusahaan',{data:''});   
        } else {
          
                res.render('perusahaan',{data:rows});
            // render to views/users/index.ejs
        }
    });
});
